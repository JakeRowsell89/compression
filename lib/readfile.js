...
const readCompressedFile = (data) => {
  const { weightPlaces, inputFrequencies, compressedText} = readCompressed(data)
  const formattedInputFrequencies = formatFrequencies(inputFrequencies, weightPlaces)
...
const compress = (err, data, target) => {
  const output = readFile(err, data, readInputFile)
